...
public class FeriaEmpresarial {
    ...
    public String metodo1() {
        ...
        int mayorZona = 0;
        int nMaxZonas[] = new int[5];
        String sZonas[] = {Puesto.ZONA_NORTE,Puesto.ZONA_SUR,Puesto.ZONA_ORIENTE,Puesto.ZONA_CENTRO,Puesto.ZONA_OCCIDENTE};
            ...
            if(puesto.estaOcupado()){
                for (int j = 0; j < empresas.size(); j++) {
                    ...
                    if(empresa.darNombre().equals(puesto.darNombreEmpresa())){
                        nPersonas = empresa.darNumeroPersonasAsistentes();
                    }
                }
                if(puesto.darZona().equals(Puesto.ZONA_NORTE)){
                    nMaxZonas[0] += nPersonas;
                }
                else if(puesto.darZona().equals(Puesto.ZONA_SUR)){
                    nMaxZonas[1] += nPersonas;
                }
                else if(puesto.darZona().equals(Puesto.ZONA_ORIENTE)){
                    nMaxZonas[2] += nPersonas;
                }
                else if(puesto.darZona().equals(Puesto.ZONA_CENTRO)){
                    nMaxZonas[3] += nPersonas;
                }
                else if(puesto.darZona().equals(Puesto.ZONA_OCCIDENTE)){
                    nMaxZonas[4] += nPersonas;
                }
            }
        }
        int indiceMayor = 0;
        int zonaMayor = 0;
        for (int i = 0; i < nMaxZonas.length; i++) {
            if(zonaMayor < nMaxZonas[i]){
                zonaMayor = nMaxZonas[i];
                indiceMayor = i;
            }
        }
            return "El mayor es " + sZonas[indiceMayor] + " con " + nMaxZonas[indiceMayor] + " personas";
    }
